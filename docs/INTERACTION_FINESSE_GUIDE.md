...
|---|------|-----------|--------------|-----------|
| 01 | 按压回弹 | 按键有重量，下压过基准再回弹 | 指令 `v-press-bounce`（`directives.ts`） | **14 个页面主操作按钮**已接入（首页/日记/相册/纪念日/留言板/矛盾/约会/足迹/消息/问答/设置/愿望/记账/待办） |
...
| 04 | 点击爆散 | 力度匹配操作，强化「按到了」 | 指令 `v-click-burst`（`directives.ts`） | **记账·保存到相册 / 愿望·完成啦 / 日记·发送评论 / 留言板·发留言 / 矛盾·标记和解 / 约会·完成啦**已接入 |
...
| 11 | 卡片抽走 | 上层移开露出下层，体现堆叠 | `SwipeCard.vue` | **愿望·已完成向左滑抽走=归档** + **待办·未完成向左滑抽走=完成**已接入 |
...
>   08 骨架落位（愿望列表加载）、09 卡片翻面（愿望卡翻面看详情）、11 卡片抽走（已完成愿望左滑归档 · 待办未完成左滑完成）
...
```
> **覆盖范围扩展**：除 14 个页面 29 处主操作按钮外，已延伸至高频内联二级动作——记账（保存总预算/添加分类预算/取消/关闭）、相册（上传图片×3/取消）、约会（取消）、消息（刷新）、设置（解除绑定/复制邀请码/加入并绑定）。原则：仅作用于"真实动作按钮"，`size="tiny"`/`quaternary`/纯文字图标按钮不铺，避免堆砌。`v-click-burst` 已覆盖的庆祝动作（保存到相册/完成啦）不重复加。
